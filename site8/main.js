...
function generateForgottenSentence() {
  const commonTemplates = [
    () => `You forgot ${getRandomThing(forgottenObjects)}.`,
    () => `You forgot to ${getRandomThing(forgottenTasks)}.`,
    () => `You forgot ${getRandomThing(randomForgottenThings)}.`
  ];
  ...
  const weight = [
    ...commonTemplates, ...commonTemplates, ...commonTemplates, // 3x weight
    ...rareTemplates
  ];

  return getRandomThing(weight)();
}

function apply() {
  document.getElementById("forgotten").innerHTML = generateForgottenSentence();
  document.getElementById("forgetBtn").innerHTML = "What did I also forget?";
}